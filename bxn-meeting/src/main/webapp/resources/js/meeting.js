...
(function (win, doc) {
    ...
    $.fn.slide = function (options) {
        ...
        if(opt.operateClass.indexOf(".")>=0){
            opt.operateClass = opt.operateClass.substring(1);
        }

        opt.offset = $('.'+opt.operateClass).width();
        ...
        return $(this).each(function () {
            ...
            $(this).on('touchend', function (e) {
                ...
                $(this).siblings().find(".memo").slideUp();
                $(this).find(".memo").slideToggle();
            ...
            }).on('click', '.'+opt.operateClass, function (e) {
                var target = $(e.target);
                ...
                opt.operateHandler&&opt.operateHandler(target,$(this));
            ...
            $(this).__proto__.slide_hideDel = function () {
            ...
            hammertime.on("pan", function (e) {
                ...
                /*
				 * if(lastAngle!=null){ if(Math.abs(180 -
				 * Math.abs(e.angle))>45){ return false; } }
				 */
...
/**
 * 获取会议的规定签到时间至规定签退时间范围
 * @param signTime
 * @param logoutTime
 */
function timeSelect(signTime,logoutTime){
	
	hoursArr = [];
	minutesArr= [];
	secondsArr = [];
	
	var beginH =  signTime.split(' ')[1].split(':')[0];
	var beginM =  signTime.split(' ')[1].split(':')[1];
	var endH =  logoutTime.split(' ')[1].split(':')[0];
	var enbM =  logoutTime.split(' ')[1].split(':')[1];
...
function getDateStr(AddDayCount) {
    var date = new Date();
    date.setDate(date.getDate() + Number(AddDayCount)); //获取AddDayCount天后的日期
 ...
 * 
 */
function changeUserStatus(id, e){
	...
	var modifyStatus = $(e).data('status');
	
	var index = $(e).parents('.slide-box').index();
	
	var pos=$('body').scrollTop();
	 
	$.ajax({
        url:'/bxn-meeting/webapi/meeting/v1/mobile/attendance-users',
        type : "POST",
        data:{
	        '_method':'PUT',
			'id':id,
        ...
        success : function(data) {
        	...
        	if($(e).parents('.weui-cell').data('page')=='detail'){
        		getMeetingInfo();
        		statusArr = [];
        	}else{
        		search();
        	...
        	setTimeout(function() {
        		// 展开当前修改的人员信息栏
        		$('#list_container .slide-box[data-id="'+id+'"]').trigger("click");
        		$('#user_container .slide-box').eq(index).trigger("click");
        		// 滚动条定位至上次的位置
        		$('body,html').animate({scrollTop:pos},500); 
        		
        		//$(e).addClass('active').parent().siblings().find('.circle').removeClass('active');
        		
        		//$(e).parents('.slide-box').find('.slide-content .weui-flex__item:last').text($(e).text()).removeClass().addClass('weui-flex__item status'+modifyStatus);
