...
<form id="meetingForm" class="input-layout-form" method="post">
		...
		<div class="panel" style="border:none">
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<input type="hidden" name="meetingId" id="meetingId" value="${meetingVo.id}">
				
					<td class="label"><span class="marker">*</span><label>会议名称：</label></td>
					<td class="content" style="width:350px;">
						<input type="text" id="meetingName" name="meetingName" value="${meetingVo.meetingName}" style="width:92%;" 
				...
				<tr  class="category_level_1">
					...
					<td class="content">
					<select id="placeId" name="placeId" style="width:330px;" >
						<c:forEach items="${placeVoList}" var="item" varStatus="i">
						
							<c:choose>
								<c:when test="${meetingVo.placeId eq item.id }">
									<option value="${item.id}" selected="selected">${item.placeName }</option>
								</c:when>
								<c:otherwise>
				...
				<tr  class="category_level_1">
					...
					<td class="content">
					<span></span>
						<input type="text" name="beginTime" id="beginTime"  required required-tip="必填开始时间" value="${meetingVo.beginTime }" style="width:150px;" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="Wdate input-fly mr20 ">
						<span>到</span>
						<input type="text" name="endTime" id="endTime"  required required-tip="必填结束时间" value="${meetingVo.endTime }" style="width:150px;" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="Wdate input-fly">
					</td>
					<td class="description"></td>
				</tr>
				<tr  class="category_level_1">
					...
					<td class="content" style="width:350px;">
					<select id="meetingTypeId" name="meetingTypeId" style="width:330px;" onchange="getMeetingGroup()">
						<c:forEach items="${meetingTypeVoList}" var="item" varStatus="i">
						<c:choose>
								<c:when test="${meetingVo.meetingTypeId eq item.id }">
				...
				<input name="cycling" type="hidden" value="${meetingVo.cycling }">
				<tr class="cycling">
					<td class="label" ><label>是否循环会议：</label></td>
					<td class="content" style="width:350px;">
						<input name="cyclingCheckbox" type="checkbox" <c:if test="${meetingVo.cycling ne 0}">checked</c:if>>
					</td>
					<td class="description"></td>
				</tr>
				
				<tr class="cycling-type" <c:if test="${meetingVo.cycling eq 0}">style="display:none"</c:if>>
					<td class="label" ><label>循环方式：</label></td>
					<td class="content" style="width:350px;">
						<input name="cyclingRadio" value="1" type="radio" <c:if test="${meetingVo.cycling eq 1}">checked</c:if>> 每日
						<input name="cyclingRadio" value="2" type="radio" <c:if test="${meetingVo.cycling eq 2}">checked</c:if>> 每周
						<input name="cyclingRadio" value="3" type="radio" <c:if test="${meetingVo.cycling eq 3}">checked</c:if>> 每月
					</td>
					<td class="description"></td>
				</tr>
						...
						<div class="panel">
							<div class="p-head">
								...
								<span class="p-head-title" id="group-container">
									<span data-id="${meetingGroupVo.id }">${meetingGroupVo.groupName }</span>
									<div class="group">
										<c:forEach items="${meetingGroupVoList}" var="item" varStatus="i">
											<span data-id="${item.id }">${item.groupName }</span>
										</c:forEach>
									</div>
								</span>
							</div>
							<div class="person-container">
								...
								<input type="hidden" id="userIds" name="userIds" />
								<c:set value="${meetingVo.userIds.split(',') }" var="userIds"></c:set>
								<c:set value="${meetingVo.userNames.split(',') }" var="userNames"></c:set>
								<c:set value="${meetingVo.uniqueNos.split(',') }" var="uniqueNos"></c:set>
								<!-- 人员列表 -->
								<div class="container-title">
									<span>已选人员（<span class="num">${fn:length(userIds) }</span>）</span>
								...
								<div class="container-content">
									<c:forEach items="${userIds}" var="id" varStatus="i">
										<div class="person-item" data-id="${id }">
											<span class="content">
												<span class="label-ico ico ico-selected-user"></span>
												<a class="value-area" href="javascript:;" id="${id }">${userNames[i.index]}(${uniqueNos[i.index]})</a>
												<a href="javascript:;" class="btn-remove" title="删除">
													<span class="ico ico-cancel3"></span>
												</a>
											</span>
										</div>
		        					</c:forEach>
				...
				<%-- <tr class="category_level_1">
					<td class="label"><span class="marker">*</span><label>人员：</label></td>
					<td class="content read-orgs">
					<input type="text" style="width: 75%;" id="addClazzStuInput" readonly="readonly" value="${meetingVo.userNames }" class="addClazzStuInput" onclick="chooseUserForStu()" />
					<input type="hidden" id="studentIds" value="${meetingVo.userIds}" name="userIds"  required required-tip="必填参会人员" style="width: 80%" invalidChar />
						<a href="javascript:;" class="btn btn-inline addClazzStu" style="vertical-align: middle;" onclick="chooseUserForStu()">
							<span class="left"></span>
							<span class="right">
								<span class="ico ico-modify"></span>
				...
				<tr class="category_level_1">
					...
					<input type="text" style="width: 75%;" id="addOrganizerInput" readonly="readonly" value="${meetingVo.organizerNames }" class="addOrganizerInput" onclick="chooseOrganizers()" />
					<input type="hidden" id="organizerIds" name="organizerIds" value="${meetingVo.organizerIds}" style="width: 80%" invalidChar/>
				...
				<tr>
					...
					<td class="content">
					<textarea name="description" id="description"  rows="3" style="width:95%; height: 100px;"  invalidChar maxlength="128">${meetingVo.description}</textarea>
					</td>
					<td class="description"></td>
				</tr>
			</table>
		...
		<div class="foot" style="">
			<a id="submitForm" href="javascript:void(0)" class="btn btn-common btn-editSave">
				<span class="left"></span>
...
<%
List<Map<String,String>> schoolList = (List<Map<String,String>>)request.getAttribute("schoolInfoList");
String schoolListJson = JsonHelper.serialize(schoolList);
%>
<%-- 从URL中提取schoolCode --%>

var schoolListJson = <%=schoolListJson%>;
...
function getMeetingGroup(){
	// 会议类型
	var meetingType = $('#meetingTypeId option:selected').val();
...
//修改保存
$("#meetingForm").on("click","a.btn-editSave",function(e){
	
	var meetingName = $("#meetingName").val();
	//var userIds = $("#studentIds").val();
	var meeetingId = $("#meetingId").val();
	var organizerIds = $("#organizerIds").val();
	var placeId = $('#placeId option:selected').val();
	var beginTime = $("#beginTime").val();
	var endTime = $("#endTime").val();
	var meetingTypeId =  $('#meetingTypeId option:selected').val();
	...
	url = "${ctx}/webapi/meeting/v1/meetings/" + meeetingId ;
	var data = "organizerIds=" + organizerIds +"&meetingName=" + meetingName  +"&placeId=" + placeId +"&meetingTypeId=" + meetingTypeId + "&cycling="+ $cycling.val() + "&userIds=" + userIds +"&beginTime=" + beginTime + "&endTime=" + endTime + "&description=" + description + "&_method=PUT";
	$.ajax({
		  cache: true,
            type: "POST",
            url:url,
