...
<body ontouchstart>
	...
	<div class="templates">
		...
		<script type="text/template" id="status_template">
			<a href="javascript:;" class="weui-grid js_grid">
				<span class="status0"><@=statusData.status0@></span>
				<p class="weui-grid__label">正常</p>
			</a>
			<a href="javascript:;" class="weui-grid js_grid">
				<span class="status1"><@=statusData.status1@></span>
				<p class="weui-grid__label">迟到</p>
			</a>
			<a href="javascript:;" class="weui-grid js_grid">
				<span class="status2"><@=statusData.status2@></span>
				<p class="weui-grid__label">早退</p>
			</a>
			<a href="javascript:;" class="weui-grid js_grid">
				<span class="status3"><@=statusData.status3@></span>
				<p class="weui-grid__label">缺勤</p>
			</a>
			<a href="javascript:;" class="weui-grid js_grid">
				<span class="status5"><@=statusData.status10@></span>
				<p class="weui-grid__label">请假</p>
			</a>
			<a href="javascript:;" class="weui-grid js_grid">
				<span class="status4"><@=statusData.status4@></span>
				<p class="weui-grid__label">漏卡</p>
			</a>
			<a href="javascript:;" class="weui-grid js_grid">
				<span class="status8"><@=statusData.status8@></span>
				<p class="weui-grid__label">迟到/早退</p>
			</a>
			<a href="javascript:;" class="weui-grid js_grid">
				<span class="status9"><@=statusData.status9@></span>
					...
					<div class="weui-panel weui-panel_access">
						...
						<div class="weui-panel__bd">
							<div class="weui-media-box weui-media-box_small-appmsg cells">
								<!-- 会议信息 -->
								<div class="weui-cells" onclick="getStatus('<@=item.meetingId@>',this)">
								...
								<!-- 考勤状态 -->
								<div class="weui-grids status-box"></div>
							</div>
						</div>
					</div>
				<@})@>
	...
	<script>
		...
		function getStatus(meetingId, e){
			var targetDom  = $(e).next('.status-box');
			// 原来是隐藏状态
			if($(targetDom).is(':hidden')){
				$.ajax({
					...
					beforeSend : function() {
						$(targetDom).html(_.template($("#loading").html()));
					},
					success : function(data) {
						statusData = data.userStatusMap;

						lastRefreshTime = statusData.lastRefreshTime;
						$(targetDom).html(_.template($("#status_template").html()));
					}
				})
			}
